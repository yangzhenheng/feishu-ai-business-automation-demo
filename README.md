...
![一键跑通截图](assets/dashboard.png)

## 飞书机器人推送截图

![飞书机器人推送截图](assets/real-feishu-webhook.png)
...
![飞书多维表格截图](assets/real-feishu-bitable.png)

## 核心能力

1. 飞书低代码多维表格设计：订单、库存、异常、客户线索、财务对账和 workflow 日志。
2. Python FastAPI 接口开发：提供 Dashboard、AI 日报、异常分类、外部 API 模拟和 V4 一键流程接口。
3. Webhook / 飞书机器人推送：AI 日报和异常摘要可推送到飞书测试群。
4. 模拟外部 API 对接：店铺订单、WMS、物流、财务、CRM。
5. SQL 查询与业务分析：支持异常订单、低库存 SKU、每日订单异常统计、高意向线索、财务差异查询。
6. AI 异常分类与处理建议：把业务异常转成结构化类型、优先级、责任部门和下一步动作。
7. AI/RAG/预测模型路线设计：客服 RAG、售后知识库、库存缺货预测、爆款 SKU 预测、财务异常识别、多模型路由。
8. 一键跑通业务流程 Demo：点击按钮即可展示完整女装电商自动化闭环。
